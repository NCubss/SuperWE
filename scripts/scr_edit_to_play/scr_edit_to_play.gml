function edit_to_play() {
	...
	if instance_exists(obj_skewer_res) {
		with (obj_skewer_res) {
			...
			switch (direct_t) {
				...
				skewer.image_xscale = scale_x;
				skewer.image_yscale = scale_y;
	...
	if (global.style == 0)
	{
	    ...
	    {
	        if (global.theme == "castle") && (!instance_exists(obj_axe_smw))
	...
	if instance_exists(obj_coin_star_res)
	{
		with (obj_coin_star_res)
		{
			visible = false
			instance_create(x, y, obj_coin_star)
		}
	}
