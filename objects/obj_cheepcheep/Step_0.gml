if (global.theme != "underwater" && modo_water == 0)
{
    ...
    if ((instance_exists(obj_lava_water) && y >= obj_lava_water.y) || (global.theme != "forest" && global.theme != "castle" && y >= (room_height - 16)))
    {
        if (paracaidas == 1)
        {
...
if (wings == 1)
{
    if (modo_vuelo == 1)
        ...
        wings_anim += 0.12
    if (global.theme == "underwater" || modo_water == 1)
    {
        if (y > (ystart + 16))
        {
            if (vspeed < -0.5)
...
if (paracaidas == 1 && (global.theme == "underwater" || (global.theme != "forest" && global.theme != "castle" && y >= (room_height - 16))))
{
    instance_create((x + (3 * direct)), y, obj_paracaidas_go)
    paracaidas = 0
    alarm[10] = 10
