...
if (vspeed >= 0)
{
    ...
    if (solidtop && bbox_bottom < (solidtop.yprevious + 5) && obj_levelmanager.editor == 0 && y > (camera_y + 16))
        y = (solidtop.y + floor(((y - bbox_bottom) - 0.9)))
    if (solidmuncher && bbox_bottom < (solidmuncher.yprevious + 5) && obj_levelmanager.editor == 0 && y > (camera_y + 16))
        y = (solidmuncher.y + floor(((y - bbox_bottom) - 0.9)))
    if (solidpinchos && bbox_bottom < (solidpinchos.yprevious + 5) && obj_levelmanager.editor == 0 && y > (camera_y + 16))
        y = (solidpinchos.y + floor(((y - bbox_bottom) - 0.9)))
    if (solidthwomp && bbox_bottom < (solidthwomp.yprevious + 5) && obj_levelmanager.editor == 0 && y > (camera_y + 16))
        y = (solidthwomp.y + floor(((y - bbox_bottom) - 16)))
    if (solidpow && bbox_bottom < (solidpow.yprevious + 5) && obj_levelmanager.editor == 0 && y > (camera_y + 16))
...
if ((collision_rectangle(bbox_left, (bbox_bottom + 1), bbox_right, (bbox_bottom + 1), obj_solidtop, 0, 0) && (!collision_rectangle(bbox_left, (bbox_bottom - 4), bbox_right, (bbox_bottom - 4), obj_solidtop, 0, 0)) && y > (camera_y + 16)) || (collision_rectangle(bbox_left, (bbox_bottom + 1), bbox_right, (bbox_bottom + 1), obj_block_pow_hold, 0, 0) && (!collision_rectangle(bbox_left, (bbox_bottom - 4), bbox_right, (bbox_bottom - 4), obj_block_pow_hold, 0, 0)) && y > (camera_y + 16)) || (collision_rectangle(bbox_left, (bbox_bottom + 1), bbox_right, (bbox_bottom + 1), obj_thwomp, 0, 0) && (!collision_rectangle(bbox_left, (bbox_bottom - 4), bbox_right, (bbox_bottom - 4), obj_thwomp, 0, 0)) && y > (camera_y + 16)) || (collision_rectangle(bbox_left, (bbox_bottom + 1), bbox_right, (bbox_bottom + 1), obj_muncher, 0, 0) && (!collision_rectangle(bbox_left, (bbox_bottom - 4), bbox_right, (bbox_bottom - 4), obj_muncher, 0, 0)) && y > (camera_y + 16)) || (collision_rectangle(bbox_left, (bbox_bottom + 1), bbox_right, (bbox_bottom + 1), obj_pinchos, 0, 0) && (!collision_rectangle(bbox_left, (bbox_bottom - 4), bbox_right, (bbox_bottom - 4), obj_pinchos, 0, 0)) && y > (camera_y + 16)) || collision_rectangle(bbox_left, (bbox_bottom + 1), bbox_right, (bbox_bottom + 1), obj_slopeparent, 1, 0))
{
    gravity = 0
    if (vspeed > 0)
        vspeed = 0
...
else if (y >= (room_height - 24) && layer_background_get_sprite(scr_bg_underwater().id) && obj_levelmanager.editor == 0)
    event_user(0)
else if (!swimming)
{
    gravity = 0.1
