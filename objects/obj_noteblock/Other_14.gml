...
if (x > (__view_get((0 << 0), 0) - 128) && (x + 16) < ((__view_get((0 << 0), 0) + __view_get((2 << 0), 0)) + 128))
{
    ...
    if (objeto && ready == 0 && objeto.bbox_bottom < (bbox_top + 5) && objeto.vspeed >= 0)
    {
        ...
        if (objeto.object_index == obj_clown || objeto.object_index == obj_clown_fire)
            spd = (spd / 10)
        else if (objeto.object_index == obj_jumppiranha && global.style == 2)
            spd = (spd / 3.5)
        objeto.vspeed = 0
        if (global.theme == "underwater" || ((global.theme == "sky" || global.theme == "airship") && global.night == 1) || (instance_exists(obj_lava_water) && objeto.y > (obj_lava_water.y + 16) && global.theme == "forest" && global.night == 0))
            objeto.vspeed = (-(spd / 2))
        else
            objeto.vspeed = (-spd)
        ready = 1
