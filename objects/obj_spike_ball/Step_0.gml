if (ready > 0)
{
    ...
    if ((ready == 1 && global.style > 1) || (ready == 1 && fisica == 1))
        event_inherited()
    else if (ready == 1 && global.style < 2)
        event_user(3)
    if (paracaidas == 0)
    {
        if (spike_alone == 0)
    ...
    if (modo_lava == 0 && instance_exists(obj_lava_water) && (bbox_bottom - 8) >= (obj_lava_water.y - 12) && global.theme == "forest" && global.night == 0)
    {
        modo_lava = 2
        if (global.style != 0)
            instance_create(x, (obj_lava_water.y - 26), obj_splash_agua)
    }
    else if ((modo_lava == 0 && instance_exists(obj_lava_water) && (bbox_bottom - 8) >= (obj_lava_water.y - 12) && global.theme == "castle") || (modo_lava == 0 && instance_exists(obj_lava_water) && (bbox_bottom - 8) >= (obj_lava_water.y - 12) && global.theme == "forest" && global.night == 1))
    {
        modo_lava = 1
        if (global.style != 0)
        {
