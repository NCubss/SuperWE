if (obj_levelmanager.editor == 1 && obj_editormanager.expand_open != 2 && ps == 1 && (!instance_exists(obj_ventana)) && obj_creator_jugar_editar.image_speed == 0)
{
    ...
    if (instance_exists(obj_apariencia) && instance_exists(obj_card_logo) && obj_apariencia.image_speed == 0)
    {
        audio_play_sound(snd_panel_close, 0, false)
        with (obj_apariencia)
            event_user(0)
    }
    else if (!instance_exists(obj_apariencia))
    {
        instance_create(camera_x, camera_y, obj_apariencia)
