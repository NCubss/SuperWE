...
{
    if (instance_exists(obj_lava_water) && (bbox_bottom - 8) > (obj_lava_water.y - 12) && (bbox_bottom - 8) < (obj_lava_water.y - 10) && (global.theme == "forest" || global.theme == "beach"))
        instance_create(x, (obj_lava_water.y - 24), obj_splash_agua)
    else if (instance_exists(obj_lava_water) && (bbox_bottom - 8) > (obj_lava_water.y - 12) && global.theme == "castle")
    {
        instance_create(x, (obj_lava_water.y - 24), obj_splash_lava)
        event_user(0)
    }
    else if (instance_exists(obj_lava_water) && (bbox_bottom - 8) > (obj_lava_water.y - 12) && global.theme == "mountain" and global.night == 1)
    {
        instance_create(x, (obj_lava_water.y - 24), obj_splash_lava)
        event_user(0)
    }
