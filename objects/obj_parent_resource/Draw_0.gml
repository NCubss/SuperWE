var drawing_shadow = surface_get_target() != application_surface;
if (obj_levelmanager.editor == 0 || outside_view())
    exit
if (drag == 1 && !drawing_shadow)
    draw_sprite_ext(grid_selected, drop_lock, x, y, 1, 1, 0, c_white, 1)
else if (mouse_up == 1 && global.cursor == 0 && !drawing_shadow)
    draw_sprite_ext(grid_selected, 0, x, y, 1, 1, 0, c_white, 1)
if (paracaidas == 1)
    draw_sprite_ext(p_sprite, global.style, (x_draw + para_x), (y_draw - para_y), s_scalex, s_scaley, 1, c_white, m_alpha)
...
if (mouse_up == 1 && drag == 0 && global.cursor == 2 && !drawing_shadow)
    draw_sprite_ext(grid_selected, 0, x, y, 1, 1, 0, c_white, 1)
