...
if (instance_exists(obj_lava_water) && bbox_bottom > (obj_lava_water.y - 12) && (global.theme == "castle" || (global.theme == "forest" && global.night == 1)))
{
    if (key == 1)
    {
...
else if ((bbox_top - 16) > room_height && global.theme != "castle" && global.theme != "forest")
{
    if (key == 1)
    {
        with (instance_create((x - 8), (y - 8), obj_key_appear))
